...
final class DataParsingTests: XCTestCase {
	...
	func testWritePadding() throws {
		do {
			...
			XCTAssertEqual([0x11, 0x00, 0x00, 0x00], Array(d.storage))
		}
		...
		do {
			...
			XCTAssertEqual([0x11, 0x22, 0x33, 0xFF], Array(d.storage))
		}
	}
	...
	func test32BitString() throws {
		...
		let writer = DataWriter()
		try writer.writeStringUTF32(message, .big)
		...
		let str2 = try parser.readUTF32String(.little, count: message.count)
