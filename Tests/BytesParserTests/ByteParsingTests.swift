...
//			let /*noise_2a*/ _: Int16 = try parser.readInteger(.little)
			...
			let title = try parser.readStringSingleByteEncodingNullTerminated(.isoLatin1)
			XCTAssertEqual("NY Times, Sunday, May 6, 2012 A-v Club", title)
			let author =  try parser.readStringSingleByteEncodingNullTerminated(.isoLatin1)
			XCTAssertEqual("Alex Vratsanos / Will Shortz", author)
			let copyright = try parser.readStringSingleByteEncodingNullTerminated(.isoLatin1)
			XCTAssertEqual("© 2012, The New York Times", copyright)

			var c = [String]()
			for _ in 0 ..< clue_count {
				let str = try parser.readStringSingleByteEncodingNullTerminated(.isoLatin1)
				c.append(str)
			}
